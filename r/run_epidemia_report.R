...
#
# Reference workflow from the EPIDEMIA / epidemiar R package
# (https://github.com/EcoGRAPH/epidemiar). The web dashboard Python pipeline
# mirrors these settings; see backend/app/services/gam_forecast.py.
#
# Expected project layout when running this script:
#   R/data_corrals.R, R/report_save_create_helpers.R
#   data/amhara_woredas.csv, data/epidemiar_settings_demo.R, ...
#   report/, epidemia_report_demo.Rnw
#
# ###############################################################################################
...
report_woredas <- read_csv("data/amhara_woredas.csv") %>%
  filter(report == 1)
# Note: report woredas must have sufficient epi data, env data, and model cluster
# information, in appropriate files
...
# req_date <- epidemiar::make_date_yw(year = 2016, week = 24, weekday = 7)
...
if (exists("epi_data") & exists("env_data")) {
  ...
  message(
    "Error: Epidemiological and/or environmental datasets are missing.
...
if (exists("pfm_reportdata") & exists("pv_reportdata")) {
  ...
  message(
    "Error: Report data for P. falciparum and/or P. vivax
